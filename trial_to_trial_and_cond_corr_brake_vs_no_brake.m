...
%% air exc inh brake vs no brake trial pair corr find
while 1
    ...
    parfor ii = 1:size(Rs,2)
        ...
        outTrialsInh{ii} = find_population_vector_corr_remap_trials(Rs(:,ii),sup(:,ii),trials);
    end
    %%
    break;
end
...
%% light exc inh brake vs no brake trial pair corr plot
while 1
    ameancorr_trials = [];
    meancorr_trials = [];
    for ii = 1:3
        toutTrials = outTrialsExc{ii};
        meancorr_trials = [meancorr_trials nanmean(exec_fun_on_cell_mat(toutTrials.adj_SP_corr_diag,'nanmean'),2)];
    end
    meancorr_trials = [mean(meancorr_trials(:,1:2),2) mean(meancorr_trials(:,3),2)];
    ameancorr_trials = meancorr_trials;
    meancorr_trials = [];
    for ii = 1:3
        toutTrials = outTrialsInh{ii};
        meancorr_trials = [meancorr_trials nanmean(exec_fun_on_cell_mat(toutTrials.adj_SP_corr_diag,'nanmean'),2)];
    end
    meancorr_trials = [mean(meancorr_trials(:,1:2),2) mean(meancorr_trials(:,3),2)];
    ameancorr_trials = [ameancorr_trials meancorr_trials];
    [within,dvn,xlabels] = make_within_table({'Type','Cond'},[2,2]);
    dataT = make_between_table({ameancorr_trials},dvn);
    ra = RMA(dataT,within);
    ra.ranova
    %%
...
%     htxt = text(0,maxY+6,sprintf('Any Condition (%d\x00B1%d%%),   All Conditions (%d%%)',round(mra),round(semra),round(mrall)),'FontSize',6);
    ...
    set(gca,'xtick',xticks,'xticklabels',xticklabels,'ytick',[0 0.03 0.06 0.09]); xtickangle(45)
    changePosition(gca,[0.09 0.01 -0.3 -0.04]); put_axes_labels(gca,{[],[0 0 0]},{{'Trial Pair','Correlation'},[0 0 0]});
...
%% air exc inh brake vs no brake trial pair corr plot
while 1
    ameancorr_trials = [];
    meancorr_trials = [];
    for ii = 1:5
    ...
    meancorr_trials = [mean(meancorr_trials(:,1:2),2) mean(meancorr_trials(:,3:5),2)];
    ameancorr_trials = meancorr_trials;
    meancorr_trials = [];
    for ii = 1:5
        toutTrials = outTrialsInh{ii};
...
%     htxt = text(0,maxY+6,sprintf('Any Condition (%d\x00B1%d%%),   All Conditions (%d%%)',round(mra),round(semra),round(mrall)),'FontSize',6);
    set_axes_limits(gca,[0.35 xdata(end)+.65],[ylims(1) maxY]); format_axes(gca);
    xticks = xdata; xticklabels = {'B-Act','NB-Act','B-Sup','NB-Sup'};
    set(gca,'xtick',xticks,'xticklabels',xticklabels,'ytick',[0 0.03 0.06]); xtickangle(45)
    changePosition(gca,[0.09 0.01 -0.3 -0.04]); put_axes_labels(gca,{[],[0 0 0]},{{'Trial Pair','Correlation'},[0 0 0]});
    save_pdf(hf,mData.pdf_folder,sprintf('light_trial_to_trial_corr.pdf'),600);
    %%
    break;
...
%% air exc inh brake vs no brake across conditions corr FIND
while 1
    %%
    si = [Ab_On Abs_On Ar_On ArL_On Ars_On];
    Rs = o.Rs(:,si);mR = o.mR(:,si);
    ntrials = 50;
    props1 = get_props_Rs(Rs,ntrials);
    exc = props1.exc;     inh = props1.inh;
    respE_OR = cell_list_op(exc,[],'or'); outE = find_population_vector_corr_remap(Rs,mR,respE_OR);
    respC_OR = cell_list_op(inh,[],'or'); outC = find_population_vector_corr_remap(Rs,mR,respC_OR);
    disp('Done');
    n = 0;
    %%
    break;
...
%% air exc inh brake vs no brake across conditions corr temporal
while 1
    for an = 1:5
        tcorr(:,:,an) = exec_fun_on_cell_mat(outE.all_SP_corr_diag{an},'mean');
    end
    for rr = 1:5
        for cc = 1:5
            if rr == cc
    ...
    mtcorr = mean(tcorr,3);
    figure(100);clf;imagesc(mtcorr)
    txli = {'B-A','B-A*','NB-A','NB-AL','NB-A*'};
    set(gca,'xtick',1:5,'ytick',1:5,'xticklabels',txli,'yticklabels',txli,'Ydir','reverse'); xtickangle(45);
    %%
